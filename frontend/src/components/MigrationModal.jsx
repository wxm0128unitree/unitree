...
export default function MigrationModal({ robot, onClose, onSubmit }) {
  ...
  return <div className="modal-mask" onClick={onClose}><div className="modal" onClick={e => e.stopPropagation()}>
    <div className="modal-kicker">调出本部门</div><h3>资产调出 · {robot.asset_code}</h3>
    <p className="modal-note">调出后不再计入部门资产统计，但全部历史记录和接收信息会保留。</p>
    ...
    <div className="actions"><button className="cancel" onClick={onClose}>取消</button><button className="danger-solid" onClick={() => department.trim() ? onSubmit({ destination_department: department.trim(), destination_holder: holder.trim(), reason: reason.trim() }) : alert('请填写接收部门')}>确认调出</button></div>
  </div></div>
}
