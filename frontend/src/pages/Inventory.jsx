...
export default function Inventory({ category, onBack, onStats, user, holders = [] }) {
  ...
  return <div className="inventory-page">
    ...
    {items.length===0?<div className="empty polished"><div className="icon">📦</div><h3>没有符合条件的{title}</h3><p>可新增配件或调整筛选条件。</p></div>:detail?<div><button className="text-btn" onClick={()=>setSelectedGroup(null)}>← 返回配件卡片</button><div className="section-heading compact"><div><h2>{detail.owner_name} · {detail.model}</h2><p>{detail.status} · 共{detail.quantity}{detail.unit} · 已编号{detail.numbered}{detail.unit} · 待编号{detail.quantity-detail.numbered}{detail.unit}</p></div></div><div className="inventory-grid">{detail.items.map(item=>{const individual=INDIVIDUAL.has(item.category);return <article className="inventory-card" key={item.id}><div className="inventory-card-head"><span className="asset-icon">{ICONS[detail.displayCategory]||'📦'}</span><div><span className="category-label">{detail.displayCategory}</span><h3>{item.asset_code||'暂未编号'}</h3></div><span className={`inventory-status status-${item.status}`}>{item.status}</span></div><div className="inventory-meta">本记录数量：{item.total_quantity}{item.unit}<span> · 持有人：{item.holder}</span><span> · 📍 {item.location||'未设置位置'}</span>{item.status==='借出'&&<span> · 借用：{item.current_borrower||item.holder}{item.current_purpose?`（${item.current_purpose}）`:''}{item.current_expected_return_at?` · 预计归还 ${new Date(item.current_expected_return_at).toLocaleDateString('zh-CN')}`:''}</span>}</div><div className="inventory-actions">{item.status==='在库'&&<>{!individual&&<button onClick={()=>setActive({item,action:'stock_in'})}>增加数量</button>}<button onClick={()=>setActive({item,action:'borrow'})}>{individual?'办理借出':'借出部分'}</button><button onClick={()=>setActive({item,action:'repair'})}>{individual?'送修':'部分转维修'}</button>{!individual&&<button onClick={()=>setActive({item,action:'migrate'})}>调出</button>}{!individual&&<button onClick={()=>setActive({item,action:'scrap'})}>减少数量</button>}</>}{item.status==='借出'&&<><button onClick={()=>setActive({item,action:'return'})}>{individual?'办理归还':'归还部分'}</button><button onClick={()=>setActive({item,action:'repair'})}>{individual?'转维修':'部分转维修'}</button></>}{item.status==='维修中'&&<button onClick={()=>setActive({item,action:'restore'})}>维修完成入库</button>}<button onClick={()=>setEditing(item)}>编辑资料</button>{user?.is_admin===1&&item.status==='在库'&&<button className="danger-link" onClick={()=>remove(item)}>作废记录</button>}</div></article>})}</div></div>:<div className="inventory-grid">{groups.map(group=><article className="inventory-card" key={group.key} onClick={()=>setSelectedGroup(group.key)}><div className="inventory-card-head"><span className="asset-icon">{ICONS[group.displayCategory]||ICONS[group.category]||'📦'}</span><div><span className="category-label">{group.displayCategory}</span><h3>{group.model}</h3></div><span className={`inventory-status status-${group.status}`}>{group.status}</span><span className="unit-pill">{group.unit}</span></div><div className="inventory-numbers"><div><b>{group.quantity}</b><span>该状态数量</span></div><div><b>{group.numbered}</b><span>已编号</span></div><div><b>{group.quantity-group.numbered}</b><span>待编号</span></div></div><div className="inventory-meta">负责人：{group.owner_name||'未指定'}{group.owner_department&&<span> · {group.owner_department}</span>}</div><div className="inventory-actions"><button onClick={e=>{e.stopPropagation();setSelectedGroup(group.key)}}>查看明细</button></div></article>)}</div>}
