...
export default function Inventory({ category, onBack, onStats, user, holders = [] }) {
  ...
  return <div className="inventory-page">
    ...
    <div className="toolbar">
      ...
      {user?.is_admin===1&&<><input className="search" list="inventory-holder-options" placeholder="持有人姓名 / 账号 / 部门" value={filters.holder==='全部'?'':filters.holder} onChange={e=>setFilters(f=>({...f,holder:e.target.value||'全部'}))}/><datalist id="inventory-holder-options">{holders.map(h=><option key={h.phone||h.name} value={h.name}>{[h.phone,h.department].filter(Boolean).join(' · ')}</option>)}</datalist></>}
    ...
    {items.length===0?<div className="empty polished"><div className="icon">📦</div><h3>没有符合条件的{title}</h3><p>可新增配件或调整筛选条件。</p></div>:<div className="inventory-grid">{items.map(item=>{const displayCategory=item.subtype||item.category;const individual=INDIVIDUAL.has(item.category);return <article className="inventory-card" key={item.id}>
      ...
      <div className="inventory-actions">{!individual&&<><button onClick={()=>setActive({item,action:'stock_in'})}>入库</button><button onClick={()=>setActive({item,action:'borrow'})}>借出</button><button onClick={()=>setActive({item,action:'return'})}>归还</button><button className="danger-link" onClick={()=>setActive({item,action:'migrate'})}>迁移</button></>}<button onClick={()=>setEditing(item)}>编辑/状态</button>{user?.is_admin===1&&<button className="danger-link" onClick={()=>remove(item)}>删除</button>}</div>
    </article>})}</div>}
