...
- 所有配件均以型号总量为基础管理，不再将电池、遥控器按单件卡片铺开。
- 在库总量可以一次借出或送修任意数量；不同借用人、使用地点或维修去向形成独立批次。
- 同一借用人、同一地点的相同配件自动合并展示，归还和维修完成支持部分数量操作。
- 旧版本已经生成的逐件记录无需迁移数据库，界面会聚合为数量批次，操作时按批次安全扣减。
- 数量拆分、批次合并和回补均保留库存流水；归还时自动合并回兼容的在库总量。
...
- 登录页使用独立品牌封面；管理界面以完整机器狗图片作为全页背景。
- 页面针对桌面和移动端做了响应式适配。
...
4. 登录页能看到品牌封面和 Unitree Logo。
5. 登录后核对机器狗背景、配件数量批次、筛选、状态操作与日志。
